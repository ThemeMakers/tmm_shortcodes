...
<ul class="carlocations_list list">
	...
	foreach ($terms as $term){
		...
		<li class="cat-item cat-item-<?php echo $term->id ?>">
			<a title="<?php echo sprintf(__('View all posts filed under %s', 'tmm_shortcodes'), $term->name); ?>" href="<?php echo $searching_page; ?>?carlocation=<?php echo $term->id ?>"><?php _e($term->name, 'tmm_shortcodes'); ?></a> (<?php echo $car_count; ?>)
			<?php
			if(count($states)){
				?>
					...
					foreach ($states as $state){
						...
						if($car_count > 0 || !$hide_empty){
							...
							<li class="cat-item cat-item-<?php echo $state->id ?>">
								<a title="<?php echo sprintf(__('View all posts filed under %s', 'tmm_shortcodes'), $state->name); ?>" href="<?php echo $searching_page; ?>?carlocation=<?php echo $term->id. ',' .$state->id ?>"><?php _e($state->name, 'tmm_shortcodes'); ?></a> (<?php echo $car_count; ?>)
								<?php
								if(count($cities)){
									?>
										...
										foreach ($cities as $city){
											...
											if($car_count > 0 || !$hide_empty){
												...
												<li class="cat-item cat-item-<?php echo $city->id ?>">
													<a title="<?php echo sprintf(__('View all posts filed under %s', 'tmm_shortcodes'), $city->name); ?>" href="<?php echo $searching_page; ?>?carlocation=<?php echo $term->id. ',' .$state->id. ',' .$city->id ?>"><?php _e($city->name, 'tmm_shortcodes'); ?></a> (<?php echo $car_count; ?>)
