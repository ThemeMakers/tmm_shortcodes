var TMM_ADVANCED_WP_POPUP2 = function () {
  var self = {
    ...
    init: function () {
      ...
      jQuery("body").prepend(
        '<div id="advanced_wp_popup_overlay" class="advanced_wp_popup_overlay_shortcode"></div>'
      );
      //***
      jQuery(document).on("click", ".advanced_wp_popup_close2", function () {
    ...
    popup: function (params) {
      ...
      jQuery("#tmm_advanced_wp_popup2").append(
        '<div class="tmm_advanced_wp_popup_li" style="width:' +
          params.width +
          "px;margin-left:-" +
          params.width / 2 +
          'px;">'
      ...
      jQuery(this.popup_li).append(
        '<div class="advanced_wp_popup_container" style="width:' +
          params.width +
          'px;">'
      );
      ...
      if (params.title.length > 0) {
        ...
        popup
          .children(".tmm_titlebar")
          .append(
            '<a href="javascript:void(0);" class="advanced_wp_popup_close2"></a>'
          );
      }
      /***/
    ...
    overlay: function (mode, zindex) {
      ...
      if (mode) {
        jQuery(
          "#advanced_wp_popup_overlay.advanced_wp_popup_overlay_shortcode"
        ).show();
      } else {
        jQuery(
          "#advanced_wp_popup_overlay.advanced_wp_popup_overlay_shortcode"
    ...
    open: function (params, popup_li) {
      ...
      jQuery.each(params.buttons, function (index, button) {
        if (button.action == "close") {
          jQuery(popup_li)
            .find(".advanced_wp_popup_buttons")
            .append(
              '<a href="javascript:void(0);" class="tmm_button advanced_wp_popup_close2">' +
                button.name +
                "</a>"
            );
          return;
        ...
        if (button.close) {
          jQuery(popup_li)
            .find(".advanced_wp_popup_buttons")
            .append(
              '<a href="javascript:tmm_advanced_wp_popup2.do_action(' +
                index +
                ');void(0);" data-name="' +
                button.name +
                '" class="tmm_button advanced_wp_popup_close2" style="display:' +
                button.display +
                ';">' +
                button.name +
                "</a>"
    ...
    set_title: function (title) {
      jQuery(this.popup_li)
        .find(".advanced_wp_popup_container .tmm_titlebar h6")
        .html(title);
    },
    show_button: function (name) {
      jQuery("#tmm_advanced_wp_popup2")
        .find(".advanced_wp_popup_buttons")
        .find("a[data-name*='" + name + "']")
    ...
    set_content: function (html) {
      return jQuery(this.popup_li)
        .find(".advanced_wp_popup_content")
        .html(html);
    },
